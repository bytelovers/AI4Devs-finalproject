...
export function parseReceiptText(text: string): ParsedReceipt {
  ...
  // Para saltar líneas de total en Fase 3 (incluye tal corrupto, no captura)
  const totalSkipRegex = /(?:total|a pagar|importe\s+total|total\s+factura|\btal\b)/i
  ...
  for (const line of lines) {
    ...
    if (kgMatch) {
      ...
      if (weight > 0 && weight < 100) {
        ...
        if (kgPrices.length >= 1) {
          ...
          const lineTotalKg = kgPrices.length >= 2 ? kgPrices[kgPrices.length - 1] : unitPriceKg * weight
    ...
    if (validPrices.length === 0) {
      ...
      const intPriceMatch = lineForParsing.match(/\b(\d{1,4})\s*$/)
      if (intPriceMatch && !hasClock && !hasDate) {
        ...
        if (letters >= 2 && digitsInName === 0 && !hasPunct && intVal > 0 && intVal <= 9999) {
          validPrices.push([intPriceMatch[0], intPriceMatch[1]])
        }
      }
