...
1. Seguridad: Debe extraer el JWT de las cabeceras de autorización o de las cookies de sesión seguras. Si el token no existe o es inválido, rechaza con HTTP 401. Extrae el `user_id` de los claims del JWT.
2. Lógica de Negocio con Turso (SQLite):
   ...
   - Si existe y `isCorrect` es TRUE: `current_box = MIN(current_box + 1, 5)`.
     Calcular `next_review_date` en base a la nueva caja: Caja 1 (+1 día), Caja 2 (+2 días), Caja 3 (+4 días), Caja 4 (+7 días), Caja 5 (+14 días). El string guardado debe tener el formato exacto `YYYY-MM-DD 00:00:00` (Inicio del día UTC).
...
3. Ejecutar los cambios en Turso mediante una sentencia `client.execute()` parametrizada. Retornar el nuevo estado del progreso.